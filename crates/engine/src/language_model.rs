...
const BACKOFF_PENALTY: i32 = 300;
const UNKNOWN_COST: i32 = 1_500;
...
impl LanguageModel {
	pub(crate) fn from_mime_bytes(
		...
		let vocabulary_end = vocabulary_offset
			.checked_add(
				vocabulary_count
					.checked_mul(VOCAB_ENTRY_SIZE)
					.ok_or(Error::InvalidDictionary(
						"language model vocabulary size overflow",
					))?,
			)
			.ok_or(Error::InvalidDictionary(
		...
		let unigram_end = unigram_offset
			.checked_add(
				unigram_count
					...
					.ok_or(Error::InvalidDictionary(
						"unigram size overflow",
					))?,
			)
			.ok_or(Error::InvalidDictionary(
				"unigram section overflow",
			))?;
		...
		let bigram_end = bigram_offset
			.checked_add(
				bigram_count
					...
					.ok_or(Error::InvalidDictionary(
						"bigram size overflow",
					))?,
			)
			.ok_or(Error::InvalidDictionary(
				"bigram section overflow",
			))?;
		...
		let trigram_end = trigram_offset
			.checked_add(
				trigram_count
					...
					.ok_or(Error::InvalidDictionary(
						"trigram size overflow",
					))?,
			)
			.ok_or(Error::InvalidDictionary(
				"trigram section overflow",
			))?;
		...
		let mut vocabulary =
			HashMap::with_capacity(vocabulary_count);

		for word_id in 0..vocabulary_count {
			let offset =
				vocabulary_offset + word_id * VOCAB_ENTRY_SIZE;

			let string_offset = read_u32(bytes, offset)
				.ok_or(Error::InvalidDictionary(
					"truncated language model vocabulary",
				))? as usize;

			let string_len = read_u32(bytes, offset + 4)
				.ok_or(Error::InvalidDictionary(
					"truncated language model vocabulary",
				))? as usize;
		...
		let mut unigrams =
			HashMap::with_capacity(unigram_count);

		for index in 0..unigram_count {
			let offset =
				unigram_offset + index * UNIGRAM_SIZE;
			...
			let cost = read_i32(bytes, offset + 4)
				.ok_or(Error::InvalidDictionary(
					"truncated unigram",
				))?;
		...
		let mut bigrams =
			HashMap::with_capacity(bigram_count);

		for index in 0..bigram_count {
			let offset =
				bigram_offset + index * BIGRAM_SIZE;
			...
			let current = read_u32(bytes, offset + 4)
				.ok_or(Error::InvalidDictionary(
					"truncated bigram",
				))?;

			let cost = read_i32(bytes, offset + 8)
				.ok_or(Error::InvalidDictionary(
					"truncated bigram",
				))?;
		...
		let mut trigrams =
			HashMap::with_capacity(trigram_count);

		for index in 0..trigram_count {
			let offset =
				trigram_offset + index * TRIGRAM_SIZE;

			let before_previous = read_u32(bytes, offset)
				.ok_or(Error::InvalidDictionary(
					"truncated trigram",
				))?;

			let previous = read_u32(bytes, offset + 4)
				.ok_or(Error::InvalidDictionary(
					"truncated trigram",
				))?;

			let current = read_u32(bytes, offset + 8)
				.ok_or(Error::InvalidDictionary(
					"truncated trigram",
				))?;
		...
		let bos_id = vocabulary
			.get(BOS_TOKEN)
			.copied();

		let eos_id = vocabulary
			.get(EOS_TOKEN)
			.copied();

		let unknown_id = vocabulary
			.get(UNKNOWN_TOKEN)
			.copied();
	...
	pub(crate) fn cost(
		...
		if let (
			...
			if let Some(cost) = self.trigrams.get(&(
				before_previous,
				previous,
				current,
			)) {
		...
		if let Some(previous) = previous {
			if let Some(cost) = self.bigrams.get(&(
				previous,
				current,
			)) {
				return cost
