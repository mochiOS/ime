...
use crate::Error;
use crate::mime::{ENTRY_SIZE, HEADER_SIZE, MAGIC, VERSION, read_i16, read_u16, read_u32, read_u64};
...
impl ConnectionMatrix {
	pub fn new(previous_size: usize, next_size: usize, costs: Vec<i16>) -> Result<Self, Error> {
		let expected = previous_size
			...
			.ok_or(Error::InvalidDictionary("connection matrix dimensions overflow"))?;
		if costs.len() != expected {
			return Err(Error::InvalidDictionary("connection matrix size does not match dimensions"));
		}
		Ok(Self {
			previous_size,
			next_size,
...
impl Dictionary {
	...
	pub(crate) fn lookup_prefix<'a>(&'a self, input: &str) -> Vec<&'a DictionaryEntry> {
		...
		for end in input.char_indices().map(|(index, ch)| index + ch.len_utf8()) {
			let reading = &input[..end];
			let start = self.entries.partition_point(|entry| entry.reading.as_str() < reading);
			let stop = self.entries.partition_point(|entry| entry.reading.as_str() <= reading);
	...
	fn from_mime_bytes(bytes: &[u8]) -> Result<Self, Error> {
		...
		let entry_count = read_u32(bytes, 8).ok_or(Error::InvalidDictionary("missing entry count"))? as usize;
		let previous_size = read_u32(bytes, 12).ok_or(Error::InvalidDictionary("missing matrix previous size"))? as usize;
		let next_size = read_u32(bytes, 16).ok_or(Error::InvalidDictionary("missing matrix next size"))? as usize;
		let entry_offset = usize::try_from(read_u64(bytes, 24).ok_or(Error::InvalidDictionary("missing entry offset"))?)
			.map_err(|_| Error::InvalidDictionary("entry offset is too large"))?;
		let string_offset = usize::try_from(read_u64(bytes, 32).ok_or(Error::InvalidDictionary("missing string offset"))?)
			.map_err(|_| Error::InvalidDictionary("string offset is too large"))?;
		let matrix_offset = usize::try_from(read_u64(bytes, 40).ok_or(Error::InvalidDictionary("missing matrix offset"))?)
		...
		if entry_bytes > bytes.len() || string_offset < entry_bytes || matrix_offset < string_offset || matrix_offset > bytes.len() {
		...
		for index in 0..entry_count {
			...
			let reading_offset = read_u32(bytes, offset).ok_or(Error::InvalidDictionary("truncated entry"))? as usize;
			let reading_len = read_u32(bytes, offset + 4).ok_or(Error::InvalidDictionary("truncated entry"))? as usize;
			let surface_offset = read_u32(bytes, offset + 8).ok_or(Error::InvalidDictionary("truncated entry"))? as usize;
			let surface_len = read_u32(bytes, offset + 12).ok_or(Error::InvalidDictionary("truncated entry"))? as usize;
			let left_id = read_u16(bytes, offset + 16).ok_or(Error::InvalidDictionary("truncated entry"))?;
			let right_id = read_u16(bytes, offset + 18).ok_or(Error::InvalidDictionary("truncated entry"))?;
			let cost = read_i16(bytes, offset + 20).ok_or(Error::InvalidDictionary("truncated entry"))?;
		...
		for index in 0..matrix_len {
			costs.push(read_i16(bytes, matrix_offset + index * 2).ok_or(Error::InvalidDictionary("truncated connection matrix"))?);
		}

		let matrix = ConnectionMatrix::new(previous_size, next_size, costs)?;
		Ok(Self::with_matrix(entries, matrix))
	}
}
...
fn read_string(table: &[u8], offset: usize, len: usize) -> Result<String, Error> {
	...
	let bytes = table
		.get(offset..end)
		.ok_or(Error::InvalidDictionary("string range is outside string table"))?;
